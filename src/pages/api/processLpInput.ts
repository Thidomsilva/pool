...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        const { text } = req.body;
        if (!text) {
            return res.status(400).json({ data: null, error: 'Nenhuma entrada fornecida. Por favor, cole o texto.' });
        }
        const parsedData = parseRawText(text);
        parsedData.uncertainFields = Object.keys(parsedData).filter(k => !parsedData[k]);
        return res.status(200).json({ data: parsedData, error: null });
    } catch (error) {
        return res.status(500).json({ data: null, error: 'Ocorreu um erro inesperado durante o processamento. Por favor, tente novamente.' });
    }
}
